...
local af = Def.ActorFrame{
	Def.Quad{
		InitCommand=function(self) self:FullScreen():Center():diffuse( ThemePrefs.Get("RainbowMode") and color("#4640a2") or Color.Black ) end,
	...
	Def.ActorFrame{
		...
		Def.ActorFrame{
			...
			Def.Sprite{
				...
				NewColorCommand=cmd(linear, delay;  diffusealpha, 0.3 ),
			},

			Def.Sprite{
				Texture=file,
				InitCommand=cmd(zoom, 1.3; x,-050; y,040; z,-100; customtexturerect,0,0,1,1; texcoordvelocity,0.04,.01 ),
				NewColorCommand=cmd(linear, delay;  diffusealpha, 0.2 ),
			},
